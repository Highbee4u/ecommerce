...
trait ApiResponder
{
    ...
    {
        return $this->showResponse(['data' => ['message' => $message, 'code' => $code]], $code);
    }

    protected function showContent($message, $code = 200)
    {
        return $this->showResponse(['data' => $message], $code);
    }
    ...
    protected function respondWithToken($token, $type)
    {
        if(isset($type)){
            return response()->json([
                'token' => $token,
                'token_type' => 'bearer',
                'message' => 'Your 2FA token as been sent to your '.($type == 'email' ? 'Email' : 'Phone')
